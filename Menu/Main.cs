    ...
    public class Main : MonoBehaviour
    {
            ...
            if (Variables.drawV2)
            {
                ...
                if (isMonkeMaterial)
                {
                    foreach (VRRig i in GorillaParent.instance.vrrigs)
                    {
                        ...
                        {
                            ii = i.mainSkin.material;
                            menuBackground.GetComponent<Renderer>().material = ii;
                        }
                    }
                }
                else
                ...
                if (Variables.isOutline)
                {
                    ...
                    if (disconnectButton)
                    {
                        ...
                        Destroy(OutlineDisconnect.GetComponent<Rigidbody>());
                        Destroy(OutlineDisconnect.GetComponent<BoxCollider>());
        ...
        public static void CreatePageButtons(float offset)
        {
            if (!triggerPage)
            {
                ...
                gameObject.transform.localScale = new Vector3(0.09f, 0.75f, 0.08f);
        ...
        {
            int lastPage = ((Buttons.buttons[Main.buttonsType].Length + Settings.buttonsPerPage - 1) / Settings.buttonsPerPage) - 1;
            Settings.buttonsPerPage = 8;
            if (UnityInput.Current.GetKeyDown(KeyCode.Q) && Main.menu != null)
            {
                Main.pageNumber--;
                if (Main.pageNumber < 0)
                {
                    Main.pageNumber = lastPage;
                }
                GorillaTagger.Instance.offlineVRRig.PlayHandTapLocal(Variables.buttonIndex, false, 0.4f);
            }
            if (UnityInput.Current.GetKeyDown(KeyCode.E) && Main.menu != null)
            {
                Main.pageNumber++;
                if (Main.pageNumber > lastPage)
                {
                    Main.pageNumber = 0;
                }
                GorillaTagger.Instance.offlineVRRig.PlayHandTapLocal(Variables.buttonIndex, false, 0.4f);
