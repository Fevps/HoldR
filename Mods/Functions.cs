...
using static Rift.Classes.Button;
namespace Rift.Mods
{
    internal class Functions
    {
        ...
        public static void RigGun()
        {
            if (ControllerInputPoller.instance.rightGrab)
            {
                ...
                if (ControllerInputPoller.instance.rightControllerIndexFloat > 0.5f)
                {
                    ...
                    GorillaTagger.Instance.myVRRig.transform.rotation = Quaternion.identity;
                }
                else
                {
        ...
        public static void NameBypasser()
        {
        ...
        {
            Hashtable hash = new Hashtable { { "mods", null } };
            PhotonNetwork.LocalPlayer.SetCustomProperties(hash);
        }

        public static void NukeModChecker()
        {
            foreach (Photon.Realtime.Player i in PhotonNetwork.PlayerList)
            {
                ...
                { 
                    { 
                        "mods",
                        @"  سارتعاشانسدارمتوازیعلیٰبیرونیپرچمبھڑکیلےمسلسلآفتشع
                            ارتعاشسانسدارمتوازیعلیٰبیرونیپرچمبھڑکیلےمسلسلآفتشع
                            ارتعاشسانسدارمتوازیعلیٰبیرونیپرچمبھڑکیلےمسلسلآفتشع
                            ارتعاشسانسدارمتوازیعلیٰبیرونیپرچمبھڑکیلےمسلسلآفتشع
                            ارتعاشسانسدارمتوازیعلیٰبیرونیپرچمبھڑکیلےمسلسلآفتشع
                            ارتعاشسانسدارمتوازیعلیٰبیرونیپرچمبھڑکیلےمسلسلآفتشع
                ...
                i.SetCustomProperties(hash);
            }
        }

        /*public static void DisableVoiceChat()
        {
            PlayerPrefs.GetString("voiceChatOn", "FALSE");
            Buttons.buttons[2][6].buttonText = "Enable Voice Chat";
            Buttons.buttons[2][6].overlapText = "Enable Voice Chat";
        }

        public static void EnableVoiceChat()
        {
            PlayerPrefs.GetString("voiceChatOn", "TRUE");
            Buttons.buttons[2][6].buttonText = "Disable Voice Chat";
            Buttons.buttons[2][6].overlapText = "Disable Voice Chat";
        }*/
        ...
        public static void CarMonke()
        {
            ...
            {
                if (CurrentCarSpeed < 15f)
                {
                    CurrentCarSpeed += 0.1f;
                }
                else
                {
                    if (CurrentCarSpeed != 15)
                    {
                        CurrentCarSpeed = 15;
                    }
                }
                Player.Instance.transform.position += Player.Instance.bodyCollider.transform.forward * Time.deltaTime * CurrentCarSpeed;
            }
            else
            {
                ...
                else if (CurrentCarSpeed < 0 || CurrentCarSpeed == 0)
                {
                    CurrentCarSpeed = 0;
                }
            }
