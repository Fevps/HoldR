...
namespace Rift.Menu
{
    internal class PlayerMods
    {
        ...
        public static void LookAtRandom()
        {
            if (ControllerInputPoller.instance.rightGrab || Mouse.current.leftButton.isPressed)
            {
                foreach (VRRig vR in GorillaParent.instance.vrrigs)
                {
                    ...
                    {
                        Movement.ena = true;
                        VRRig ii = RigManager.GetClosestVRRig();
                        GorillaTagger.Instance.offlineVRRig.enabled = false;
        ...
        public static void SpazHeadAtEveryone()
        {
            if (PhotonNetwork.InRoom)
            {
                Photon.Realtime.Player o = RigManager.GetRandomPlayer(false);
                VRRig i = RigManager.GetVRRigFromPlayer(o);
                GorillaTagger.Instance.headCollider.transform.LookAt(i.transform.position);
            }
        }
    }
}
